...
app.controller('mainController', function($timeout, $q, $log, $http, $scope, $window) {
	...
	function generateIndicatorGraph(title,func,valueArray,keyArray,timeArray,symbol){
		var colorArray = ["#3399ff", "black", "#7CFC00", "#191970"];
		var seriesArray = [];
	...
	function generateIndicatorData(response,func){
		...
		var keyArray = Object.keys(values[0]);
		var valueArray = new Array(keyArray.length);
	...
	function getIndicators(){
		var funcs = ["SMA","EMA","STOCH","RSI","ADX","CCI","BBANDS","MACD"];
		for(var i in funcs){
			//console.log(funcs[i]);
